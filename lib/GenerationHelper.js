...
function getPacketTypehash()  public pure returns (bytes32) {
    ...
    static getCustomTypedDatahashMethod(customConfig){
        ...
        output = output.concat('\n')
        output = output.concat(`getEIP712DomainHash('${customConfig.contractName}','${customConfig.version}',getChainID(),address(this)),`)
        output = output.concat('\n')
    ...
    static getVerificationMethod(customConfig){
        ...
        let output = `function verifyOffchainSignatureAndDoStuff(${middleLine},bytes memory offchainSignature) public view returns (bool) {`
        output = output.concat('\n')
        ...
        output = output.concat('\n')
        output = output.concat(`require(orderCreator == recoveredSignatureSigner, 'Invalid signature');`)
        output = output.concat('\n')
